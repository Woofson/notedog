...
pub fn render_help_modal(f: &mut Frame, area: Rect, scroll_y: usize, theme: &Theme) {
    ...
    let help_lines = vec![
        ...
        Line::from(vec![Span::raw("  Tab / Shift+Tab   Toggle focus between Notes list and Main View (Superfile-style)")]),
        Line::from(vec![Span::raw("  s                 Jump focus directly to Sections pane")]),
        Line::from(vec![Span::raw("  b                 Jump focus directly to Notebooks pane")]),
        Line::from(vec![Span::raw("  n                 Jump focus directly to Notes pane")]),
        Line::from(vec![Span::raw("  1 / 2 / 3 / 4     Jump to Notebooks (1), Sections (2), Notes (3), Viewer (4)")]),
        Line::from(vec![Span::raw("  ← / → or h / l    Horizontal pane navigation (or switch Notebook tabs)")]),
        Line::from(vec![Span::raw("  Enter             Step inside section / Open note editor")]),
        Line::from(vec![Span::raw("  F1                Cycle active Notebook tab")]),
        Line::from(vec![Span::raw("  ↑ / ↓ / k / j     Vertical navigation in active list / Scroll note preview")]),
        ...
        Line::from(""),
        Line::from(vec![Span::styled("PASSWORD PROTECTION & LOCKING", Style::default().fg(theme.encrypted_tag).add_modifier(Modifier::BOLD))]),
        Line::from(vec![Span::raw("  u / Ctrl+U        Unlock focused Notebook, Section, or Note for this session")]),
        Line::from(vec![Span::raw("  Ctrl+L            Instant Lock (Clear session password and lock all notes)")]),
        Line::from(vec![Span::raw("  Ctrl+E            Set Password Protection / Remove Protection (with 2-step confirmation)")]),
        Line::from(vec![Span::raw("  Ctrl+P            Change Password for focused Notebook, Section, or Note")]),
        Line::from(""),
