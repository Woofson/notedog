...
pub fn render_help_modal(f: &mut Frame, area: Rect, scroll_y: usize, theme: &Theme) {
    ...
    let help_lines = vec![
        ...
        Line::from(vec![Span::raw("  Ctrl+E            Contextual Encrypt/Decrypt (with 2-step confirmation)")]),
        Line::from(vec![Span::raw("  Ctrl+P            Change Passphrase for focused Notebook, Section, or Note")]),
        Line::from(""),
